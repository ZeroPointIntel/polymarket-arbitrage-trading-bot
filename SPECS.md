...
│   ├── exec/
│   │   ├── OrderRouter.{h,cpp}         # FAK orders, retry, idempotency
...
- Kernel: pin trading thread to dedicated cores via `isolcpus` + `taskset`
- Disable C-states (`intel_idle.max_cstate=0 processor.max_cstate=0`)
- CPU governor: `performance`
- NIC: enable busy-polling, increase ring buffer sizes, RSS to dedicated cores
- Disable transparent hugepages on the trading process
