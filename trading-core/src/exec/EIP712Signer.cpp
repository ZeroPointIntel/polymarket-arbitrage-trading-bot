...
void EIP712Signer::compute_domain_separator() {
    ...
    auto version_hash = crypto::Keccak256::hash("1");
...
std::array<uint8_t, 32> EIP712Signer::hash_order(const Order& order) const {
    ...
    const std::string order_type = "Order(uint256 salt,address maker,address signer,address taker,uint256 tokenId,uint256 makerAmount,uint256 takerAmount,uint256 expiration,uint256 nonce,uint256 feeRateBps,uint8 side,uint8 signatureType)";
    ...
    push_bytes(hex_to_bytes32(order.taker));
    ...
    push_bytes(uint256_to_bytes32(order.expiration));
    push_bytes(uint256_to_bytes32(order.nonce));
    push_bytes(uint256_to_bytes32(order.feeRateBps));
