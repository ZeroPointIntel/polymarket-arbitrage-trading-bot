...
void SignalEngine::handle_latency_signal(const LatencyArbSignal& sig) {
    ...
    double num_shares = trade_size_usd_ / sig.polymarket_price;
    double cost_usdc = num_shares * sig.polymarket_price;

    auto [can_open, reason] = risk_manager_->can_open_position(cost_usdc);

    if (can_open) {
        spdlog::info("SignalEngine: Approved LA order {} for token {} at {:.3f}", order_id, sig.token_id, sig.polymarket_price);
...
void SignalEngine::handle_dump_hedge_signal(const DumpHedgeSignal& sig) {
    ...
    double num_shares = trade_size_usd_ / sig.combined_price; 
    double combined_cost_usdc = num_shares * sig.combined_price;

    auto [can_open, reason] = risk_manager_->can_open_dh_position(combined_cost_usdc);

    if (can_open) {
        spdlog::info("SignalEngine: Approved DH pair YES: {:.3f} NO: {:.3f} for asset {}", sig.yes_price, sig.no_price, sig.asset);
