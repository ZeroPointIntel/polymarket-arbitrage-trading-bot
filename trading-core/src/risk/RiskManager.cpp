...
void RiskManager::register_trade_open(const Position& position) {
    ...
    current_balance_ -= position.cost_usdc;

    spdlog::info("Position OPENED | {} | ${:.2f} USDC | Balance: ${:.2f}",
                 position.order_id, position.cost_usdc, current_balance_);
}
...
void RiskManager::register_dh_open(const DumpHedgePosition& position) {
    ...
    current_balance_ -= position.combined_cost_usdc;

    spdlog::info("DH Position OPENED | {} | ${:.2f} USDC | Locked: ${:.2f} | Balance: ${:.2f}",
                 position.dh_id, position.combined_cost_usdc, position.locked_profit_usdc, current_balance_);
}
...
std::optional<DumpHedgePosition> RiskManager::register_dh_close(
    ...
    double pnl = 0.0;
    if (actual_proceeds_usdc.has_value()) {
        pnl = actual_proceeds_usdc.value() - pos.combined_cost_usdc;
        current_balance_ += actual_proceeds_usdc.value();
    } else {
        double combined_exit = yes_exit_price + no_exit_price;
        pnl = (combined_exit - pos.combined_entry_price) * pos.size_shares;
        current_balance_ += pos.combined_cost_usdc + pnl;
    }
...
void RiskManager::check_risk_thresholds() {
    ...
    if (peak_balance_ > 0) {
        double drawdown_pct = (peak_balance_ - current_balance_) / peak_balance_;
        if (drawdown_pct >= total_drawdown_kill_) {
            trigger_kill_switch(
                "Total drawdown " + std::to_string(drawdown_pct * 100.0) + "% exceeded threshold. " +
                "Peak: $" + std::to_string(peak_balance_) + " -> Current: $" + std::to_string(current_balance_)
            );
            return;
        }
    }
    ...
    if (daily_starting_balance_ > 0) {
        double daily_loss_pct = (daily_starting_balance_ - current_balance_) / daily_starting_balance_;
        if (daily_loss_pct >= daily_loss_limit_) {
            trigger_daily_halt(
                "Daily loss " + std::to_string(daily_loss_pct * 100.0) + "% exceeded limit. " +
                "Daily start: $" + std::to_string(daily_starting_balance_) + " -> Current: $" + std::to_string(current_balance_)
            );
        }
    }
}
